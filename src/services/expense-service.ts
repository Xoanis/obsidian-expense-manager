	...
	async parseTransactionFile(file: TFile): Promise<TransactionData | null> {
		try {
			...
			return {
				...
				emailMessageId: frontmatter.email_msg_id as string | undefined,
				emailProvider: frontmatter.email_provider as string | undefined,
				emailMailboxScope: frontmatter.email_mailbox_scope as string | undefined,
				duplicateOf: frontmatter.duplicate_of as string | undefined,
	...
	private async createTransactionViaParaCore(data: TransactionData): Promise<TFile> {
		...
		const file = await this.paraCoreApi.createNote({
			...
			frontmatterOverrides: {
				...
				tags,
				email_msg_id: data.emailMessageId ?? null,
				email_provider: data.emailProvider ?? null,
				email_mailbox_scope: data.emailMailboxScope ?? null,
